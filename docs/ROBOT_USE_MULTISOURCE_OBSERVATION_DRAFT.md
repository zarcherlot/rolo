...
- [`ARCHITECTURE.md`](ARCHITECTURE.md) 已把 `robot_use` 放在 Diagnose 阶段，并明确完整闭环仍待实现；
...
- [`SENSITIVE_INVOCATION_POLICY.md`](SENSITIVE_INVOCATION_POLICY.md) 已建立图像、地图和空间数据的默认拒绝、主机身份授权与无业务 payload 审计边界。
