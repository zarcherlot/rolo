...
状态：参考文档（`active`）  
权威级别：实现索引，不覆盖 Contract、Schema、安全门禁或发布规则  
维护者：ROLO maintainers  
最后复核：2026-08-29

本文回答“当前代码在哪里实现、如何串起来、由什么测试保护”。它不重新定义业务概念。
遇到冲突时，优先遵循 [最高开发准则](DEVELOPMENT_PRINCIPLES.md)、版本化 Schema、
Operation Contract、运行时 Gate 及其测试；阶段契约以对应的 Episode/Diagnose/Verify
文档为准。
...
| HTTP 控制面 | `src/rolo/api.py` | `/v1` 只读查询、阶段 run/取消、授权请求和 read model API |
| `rolo-mcp` | `src/rolo/mcp_server.py` | MCP/Agent 访问控制面和受限工具，不绕过服务端 Gate |

CLI 的公开脚本定义位于 `pyproject.toml`。新增入口必须同时说明输入、产物、失败关闭
行为和对应测试，不能只在 CLI help 中出现。
...
`src/rolo/stages/agent_runner.py` 是 Diagnose/Verify 共用的执行边界，负责任务摘要、
授权、幂等、运行状态、日志流、取消、租约恢复和 handoff validator 回调。它本身不授予
release 或 acceptance authority。
...
| 规划与 Agent | `journey.py`、`service.py`、`agent_runner.py`、`agent_provider.py` | 构造有摘要绑定的任务；Agent 工作区是临时的，不是权威来源 |
...
Adapt 的核心不变量是“Agent 提案 ≠ Rolo 权威”：Agent 不能写 Tool Catalog，不能用静态
声明证明物理行为、可靠性或安全性；独立 Gate 和运行时校验才是发布与调用边界。
详细规则见 [三阶段架构](ARCHITECTURE.md)、[Agent-native Tools](AGENT_NATIVE_TOOLS.md)
和 [Registry Operation 指南](REGISTRY_OPERATION_GUIDE.md)。
...
本地无目标机的开发路径见 [LOCAL_DIAGNOSE_VERIFY_FAKE.md](LOCAL_DIAGNOSE_VERIFY_FAKE.md)。
...
Registry 的字段、Contract SHA、迁移关系和门禁仍以 Registry 文档及生成目录为准；本文只
标出代码位置，避免再次复制 294 项定义。
...
或 remediation。详细字段和状态以对应的 Episode contract 文档为准。
...
独立 Conformance Gate、不可变 Adapt release、Diagnose/Verify Stage Agent envelope、
授权与恢复、Episode 只读 read model，以及 Native Tool 的受控观测通道。
