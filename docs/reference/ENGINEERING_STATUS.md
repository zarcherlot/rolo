...
| FEAT-APPLICATION-MAP-EXPLORE-L1 | PARTIAL | E4 | `rolo target application-map-explore-plan --profile`; `rolo target application-map-explore --profile` | `src/rolo/stages/probe/application_exploration.py`; `scripts/rolo_micro_explorer.py`; `src/rolo/targets/executor.py`; `src/rolo/product_cli.py` | `tests/test_application_exploration.py` | 已在 mentorpi 完成一次 3.5 秒低速微探索并复核末尾零速；仍不证明完整覆盖、避障正确性或长期安全 |
