...
| FEAT-MOTION-SAFETY-CONFORMANCE | PARTIAL | E3 | `rolo target safety-conformance --profile`; target-side `scripts/rolo_ros_safety_arbiter.py` | `src/rolo/stages/probe/application_safety.py`; `src/rolo/stages/probe/safety_guard.py`; `src/rolo/product_cli.py`; `scripts/rolo_ros_safety_arbiter.py` | `tests/test_application_safety.py`; `tests/test_safety_guard.py` | 已有 ROS-independent guard core 和可部署 ROS 2 arbiter（限速、前方障碍、scan/command watchdog）；LanderPi 尚未部署并将控制器重映射到 safe output，独立急停仍明确不在本阶段 |
