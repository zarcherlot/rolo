...
| FEAT-APPLICATION-MAP-EXPLORE-L1 | PARTIAL | E3 | `rolo target application-map-explore-plan --profile`; `rolo target application-map-explore --profile` | `src/rolo/stages/probe/application_exploration.py`; `scripts/rolo_micro_explorer.py`; `src/rolo/targets/executor.py`; `src/rolo/product_cli.py` | `tests/test_application_exploration.py` | 计划和归零链已在 mentorpi 验证；L2 执行现要求独立运动反馈（如 `/odom_rf2o`）才能避免把命令积分误当物理位移；当前目标缺少该 publisher，因此不再允许误报物理运动 |
