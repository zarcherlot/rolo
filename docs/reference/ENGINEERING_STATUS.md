...
| FEAT-ADAPT-GATE-RELEASE | STABLE | E2 | Adapt bundle、conformance、immutable release、目标 CLI sandbox 可见性校验 | `src/rolo/stages/adapt/service.py`; `src/rolo/stages/adapt/conformance.py`; `src/rolo/adapter_runtime.py` | `tests/test_conformance.py`; `tests/test_adapter_runtime.py`; `tests/test_adapt_journey.py` | Adapt 不证明物理结果、可靠性或安全性；目标机仍需验证实际 PATH、shebang 和 sandbox |
| FEAT-ADAPT-ROUTE-MAPPING | PARTIAL | E3 | Registry-aware semantic mapping、route evidence、CLI help probe、sandbox 可见性和多路由 selector | `src/rolo/stages/adapt/application_cli_mapping.py`; `src/rolo/stages/adapt/semantic_mapping.py`; `src/rolo/stages/adapt/mapping_evidence_tool.py`; `src/rolo/stages/adapt/routes.py`; `src/rolo/adapter_runtime.py` | `tests/test_heuristic_discovery.py`; `tests/test_ros_probe.py`; `tests/test_operation_eligibility.py`; `tests/test_state_graph.py`; `tests/test_adapter_runtime.py` | 启发式/静态映射仍是候选证据；help probe 只证明有界自描述；promotion 会拒绝 sandbox 内不可解析的 executable/interpreter；多路由调用必须显式 selector |
...
| FEAT-VERIFY-CONTRACT | PARTIAL | E3 | Verify plan、acceptance plan、evidence、replay 和 real-target readiness gate | `src/rolo/stages/verify/service.py`; `src/rolo/stages/verify/acceptance.py`; `src/rolo/stages/verify/readiness.py`; `src/rolo/stages/verify/legacy_adapter.py`; `src/rolo/stages/verify/ssh_provenance.py`; `src/rolo/stages/verify/ssh_target_provider.py`; `src/rolo/stages/real_target.py` | `tests/test_verification_acceptance.py`; `tests/test_verify_readiness.py`; `tests/test_legacy_provider_adapter.py`; `tests/test_post_r5_provider_boundary.py`; `tests/test_ssh_provenance.py`; `tests/test_ssh_target_provider.py`; `tests/test_ssh_provider_handoff.py`; `tests/test_ssh_provider_recovery.py`; `tests/test_handoff_materializers.py`; `tests/test_real_target_contracts.py` | Readiness 只校验 canonical v2 provider/provenance/evidence 绑定，不授予 release authority；真实目标、物理机器人和通用 acceptance oracle 仍需真机验证 |
...
| FEAT-LOCAL-REAL-TARGET | PARTIAL | E3 | profile 绑定的 Linux/ROS Diagnose/Verify provider（local/SSH） | `src/rolo/stages/real_target.py`; `src/rolo/targets/profiles.py`; `src/rolo/stages/verify/ssh_provenance.py` | `tests/test_real_target_contracts.py`; `tests/test_target_profiles.py`; `tests/test_ssh_provenance.py`; `tests/test_ssh_profile_stage_executor.py`; `tests/test_ssh_profile_real_e2e.py` | SSH 依赖 pinned known_hosts 内容摘要、与 host/port 匹配的已批准 host key、`ssh-agent:default` 或摘要校验 identity file 和可达目标；其他 secret-store/keychain resolver、物理机器人 E4 闭环仍未完成 |
