...
| FEAT-APPLICATION-MAP-EXPLORE-L1 | PARTIAL | E2 | `rolo target application-map-explore-plan --profile` | `src/rolo/stages/probe/application_exploration.py`; `scripts/rolo_micro_explorer.py`; `src/rolo/product_cli.py` | `tests/test_application_exploration.py` | 仅生成固定低速、短时、每段归零的计划；当前不执行运动，执行器仍需安全 Conformance 和单独确认 |
