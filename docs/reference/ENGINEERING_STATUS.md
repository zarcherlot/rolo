...
| FEAT-STAGE-RUNNER | STABLE | E3 | Diagnose/Verify 授权、幂等、取消和日志 | `src/rolo/stages/agent_runner.py`; `src/rolo/stages/downstream.py`; `src/rolo/user_identity.py` | `tests/test_stage_agent_runner.py`; `tests/test_stage_agent_read_models.py`; `tests/test_user_identity.py` | Runner 不决定业务结论或 acceptance；本地 session 绑定依赖 artifact root 的持久 session 文件 |
