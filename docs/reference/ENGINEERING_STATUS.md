...
| FEAT-STAGE-RUNNER | STABLE | E3 | Diagnose/Verify 授权、幂等、取消、日志和重启恢复维护 | `src/rolo/stages/agent_runner.py`; `src/rolo/stages/downstream.py` | `tests/test_stage_agent_runner.py`; `tests/test_stage_agent_read_models.py`; `tests/test_stage_recovery_maintenance.py` | Runner/maintenance 不决定业务结论或 acceptance；真实进程 kill/reboot 仍需目标机注入 |
