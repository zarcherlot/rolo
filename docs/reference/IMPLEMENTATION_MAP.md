...
| Application gap bundle | `src/rolo/stages/probe/application.py`, `application_write.py`, `application_mapping.py`, `application_exploration.py`, `application_safety.py`, `src/rolo/product_cli.py` | Derives four small-car candidates, exposes the no-motion `app.map.create` session start, emits a bounded L1 exploration plan, and independently checks the safety routes required before execution |
