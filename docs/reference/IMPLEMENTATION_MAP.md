...
| HTTP 控制面 | `src/rolo/api.py` | `/v1` 查询、阶段 run/取消、授权请求、read model API 和实验性的 sanitized artifact registration；非 loopback 请求使用 Bearer token，并对受保护 read/write endpoint 执行显式 scope 校验 |
...
| 固定 Linux/ROS 目标、SSH profile 与 Stage 插件 | `test_real_target_contracts.py`、`test_ssh_profile_stage_executor.py`、`test_plugin_manifest.py` |
