...
本次随后执行 L2 计划（用户确认 `I CONFIRM APP.MAP.EXPLORE.L2`）：目标命令链确实
运行了直行/旋转/归零序列，`/map` 栅格从约 `67 x 62` 扩展到 `67 x 64`，执行进程随后
退出，`/controller/cmd_vel_safe` 复查为全零。但用户现场确认小车没有发生物理运动；
复盘发现目标 `/odom` 来自 `odom_publisher` 对命令速度的积分，而独立运动反馈
`/odom_rf2o` 当前没有 publisher。因此此前将 `/odom` 变化描述为真实位移是错误的，
本次 L2 不计为物理运动通过，也不等于完整环境建图完成。

后续 L2 执行已增加独立运动反馈硬门：没有可观测的编码器/视觉/激光里程计 publisher
时直接拒绝，不再把指令积分当作物理运动证据。
