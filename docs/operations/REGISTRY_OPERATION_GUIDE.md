...
- [Operation 词汇历史审计](../archive/operations/OPERATION_TAXONOMY_AUDIT.md)：重复、近义和边界审计；
- [SENSITIVE_INVOCATION_POLICY.md](SENSITIVE_INVOCATION_POLICY.md)：SENSITIVE、R2/R3 运行时授权；
- [SOFTWARE_DISCOVERY.md](../adapt/SOFTWARE_DISCOVERY.md)：candidate 与 probe 证据边界。

## 12. 当前治理摘要

以下规则是当前实现的最小治理面，完整历史讨论见归档审计：

- 一个稳定产品意图只保留一个 Canonical Operation；仅转发 Linux/ROS/HW 命令的 wrapper
  不应新增为产品 operation；
- `status` 与 `health`、`plan/validate` 与 `execute/apply`、普通 stop 与安全 stop
  只有在权限、返回语义或安全路径确实不同的情况下才保持分离；
- 同一 operation 的兼容扩展优先使用向后兼容参数和契约版本升级，破坏性变化必须经过迁移窗口；
- 发现层只能产生候选和证据，不能直接提升 `VERIFIED` 或绕过独立 Gate；
- 新增或修改 operation 必须同步 YAML、Schema、生成文档、测试、conformance 和迁移记录。
