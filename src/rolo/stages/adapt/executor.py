    ...
    def _build_prompt(
        ...
        *,
        ...
        return (
            ...
            "and a bindings array. Every binding item includes operation, route_id, resource_id, "
            "kind, "
            ...
            "item's route_id, resource_id, endpoint and semantic_bindings to select exactly one "
            "route deterministically; Rolo may supply selected_route_id after resolving an input "
            "selector. Fail closed if the request cannot identify one or identifies more than one. "
            "Reject a "
            ...
            "ROLO_TARGET_EXECUTABLE configuration value; release validation rejects collector "
            "paths copied into source. "
            "Never hardcode a ROS topic, service, action, device path, or vendor interface in "
            "the adapter package. At invocation time Rolo supplies the operation-scoped JSON "
            "document in ROLO_TARGET_ROUTE_BINDINGS_JSON; parse and validate that document, "
            "resolve the endpoint from its bindings, and fail closed when it is absent, "
            "ambiguous, or mismatched. This keeps the adapter portable across robot platforms "
            "and software stacks. "
