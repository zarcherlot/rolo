...
def _bounded_context(value: dict[str, Any]) -> dict[str, Any]:
    """Fit selected evidence to a real serialized-character budget."""
    ...
    def lists(item: Any) -> list[list[Any]]:
        found: list[list[Any]] = []
        if isinstance(item, dict):
            for child in item.values():
                found.extend(lists(child))
        elif isinstance(item, list):
            found.append(item)
            for child in item:
                found.extend(lists(child))
    ...
    def nested_dicts(item: Any, *, root: bool = False) -> list[dict[str, Any]]:
    ...
    original_chars = len(json.dumps(result, ensure_ascii=False, separators=(",", ":")))
    truncate_strings(result)
    target_chars = MAX_AGENT_CONTEXT_CHARS - 500
    while len(json.dumps(result, ensure_ascii=False, separators=(",", ":"))) > target_chars:
        list_candidates = [item for item in lists(result) if item]
        dict_candidates = [item for item in nested_dicts(result, root=True) if len(item) > 1]
        candidates: list[Any] = [*list_candidates, *dict_candidates]
        if not candidates:
            break
        largest = max(
            candidates,
        ...
        if isinstance(largest, list):
            ...
            for key in keys[max(1, len(keys) // 2) :]:
                largest.pop(key, None)
    result["context_budget"] = {
        "max_chars": MAX_AGENT_CONTEXT_CHARS,
        "original_chars": original_chars,
        "truncated": original_chars > MAX_AGENT_CONTEXT_CHARS,
    }
    if len(json.dumps(result, ensure_ascii=False, separators=(",", ":"))) > MAX_AGENT_CONTEXT_CHARS:
        result = {
            key: result.get(key)
            for key in ("robot_id", "discovery_id", "status", "platform")
            if key in result
        } | {
            "context_budget": {
                "max_chars": MAX_AGENT_CONTEXT_CHARS,
                "original_chars": original_chars,
                "truncated": True,
                "fallback": "identity-and-platform-only",
            }
        }
    return result
