    ...
    if isinstance(proposals, list):
        for proposal in proposals:
            ...
            if isinstance(supporting, list) and isinstance(counter, list):
                # Counter evidence cannot overlap supporting evidence. Removing
                # an impossible overlap cannot create semantic ACCEPT: exact
...
class CodexOperationMappingProvider:
    ...
    def _batch_request(
        ...
        contracts = [item for item in request.target_contracts if item.operation in selected]
        bindings = {
            operation: binding
