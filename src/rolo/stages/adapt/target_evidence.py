...
# application workspace (LeRobot currently exposes more than four console
# entrypoints).  Active discovery applies its own 20-process probe budget.
...
def discover_help_executables(project_root: Path) -> list[Path]:
    ...
    Collector enrollment used to require a hand-written allowlist, and the
    product journey consequently carried only whichever one executable the
    operator happened to name.  This helper only inspects the project metadata
    and conventional virtualenv/install ``bin`` directories; the resulting
    paths are still pinned and hashed by :func:`initialize_collector` before
    any later ``--help`` invocation.
    """
...
def refresh_local_deployment(
    *,
    ...
    Existing deployments are intentionally immutable during a normal Adapt
    start.  This helper makes the safe expansion workflow explicit: discover
    only project-declared/conventional installed entrypoints, pin their paths
    and digests in a fresh collector, then re-enroll the deployment while
    preserving an immutable transition record.
    """
    ...
    # A refresh is additive by design.  An operator-supplied override may add
    # entries, but it must not silently drop an already pinned executable.
    ...
    try:
        ...
        # The staged key/state are not active if re-enrollment fails.  Remove
        # only these exact files; the previous pinned deployment remains intact.
...
def _collect_executable_help(
    ...
    for executable_id in request.requested_executable_help_ids:
        ...
        if not path.is_file() or path.stat().st_size > MAX_HELP_EXECUTABLE_BYTES:
            # A single stale/removed executable must not suppress evidence for
            # every other pinned CLI.  Preserve the failure as signed evidence
            # so route binding simply ignores this item and the gate can still
            # use successfully inspected siblings.
