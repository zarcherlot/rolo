...
def infer_application_cli_operations(
    ...
    *,
    ...
    for definition in canonical_operation_registry().operations:
        ...
        if definition.access == "read" and endpoint_tokens & _SIDE_EFFECTING_ENDPOINT_TOKENS:
            continue
        # Emergency stop requires explicit emergency-stop intent rather than
        # the generic lexical ``stop`` class.
