...
class CodingAgentDependencyManager:
    """Resolve, install, verify, and audit the configured Coding Agent dependency."""
    ...
    def prepare(
        ...
        *,
    ...
    ) -> tuple[CodingAgentDependencyReport, Path]:
        ...
        if install_timeout_s < 1:
            report = self._report(
                config=config,
                status=CodingAgentDependencyStatus.FAILED,
                system=system,
                architecture=architecture,
                messages=["Coding Agent install timeout must be at least one second"],
            )
        ...
        if config.executor.strip().lower() != self.adapter.name:
            report = self._report(
                config=config,
                status=CodingAgentDependencyStatus.UNSUPPORTED,
                system=system,
                architecture=architecture,
        ...
        if resolved is None:
            status = (
                CodingAgentDependencyStatus.FAILED
                if install_attempted
                else CodingAgentDependencyStatus.INSTALL_REQUIRED
            )
            messages.append("Configured Coding Agent executable is not installed")
            report = self._report(
                config=config,
                status=status,
                system=system,
        ...
        if version is None:
            report = self._report(
                ...
                installed=True,
                install_attempted=install_attempted,
                messages=["Coding Agent executable did not return a valid version"],
            )
            return report, self._write(report)
        ...
        if not require_auth:
            report = self._report(
                config=config,
                status=(
                    CodingAgentDependencyStatus.INSTALLED
                    if install_attempted
                    else CodingAgentDependencyStatus.READY
                ),
                system=system,
                architecture=architecture,
                executable=str(resolved),
        ...
        if config.api_key_configured:
            report = self._report(
                config=config,
                status=CodingAgentDependencyStatus.READY,
                system=system,
                architecture=architecture,
                executable=str(resolved),
                version=version,
        ...
        report = self._report(
            config=config,
            status=(
                CodingAgentDependencyStatus.READY
                if authenticated
                else CodingAgentDependencyStatus.AUTH_REQUIRED
            ),
            system=system,
            architecture=architecture,
            executable=str(resolved),
    ...
    def _report(
        ...
        *,
        ...
        return CodingAgentDependencyReport(
    ...
    def _write(self, report: CodingAgentDependencyReport) -> Path:
