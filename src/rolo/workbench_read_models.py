...
def _add_episode_evidence(
    records: dict[str, EvidenceRecord],
    artifact_root: Path,
    robot_id: str,
) -> None:
    from rolo.episode_projection import episode_evidence_specs

    for spec in episode_evidence_specs(artifact_root, robot_id).values():
        _add_evidence(
            records,
            EvidenceRecord(
                evidence_id=spec.evidence_id,
...
def build_evidence_collection(
    ...
    *,
    ...
    if artifact_root is not None:
        ...
        _add_episode_evidence(records, artifact_root, robot.robot_id)
