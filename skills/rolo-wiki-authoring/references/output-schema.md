...
`robot-wiki-insights/v1` for one migration cycle. New output must never select the legacy version. The
canonical export is `schemas/RoloWikiInsightBundle.schema.json`.
...
and ensure Wiki content does not enter eligibility, Catalog or release digests. Before merging, call
`validate_rolo_wiki_insights` with a caller-owned `RoloWikiValidationContext`; the input digest map,
target/release/conformance identities and all referenced evidence paths must match exactly.
