...
def _prepare_promotion(
    ...
    *,
    ...
    package_path.write_text(
        ...
        "elif sys.argv[1] == 'validate-binding':\n"
        "    args = dict(zip(sys.argv[2::2], sys.argv[3::2]))\n"
        "    operation = args.get('--operation')\n"
        "    entrypoint = args.get('--entrypoint')\n"
        "    document = json.loads(os.environ['ROLO_TARGET_ROUTE_BINDINGS_JSON'])\n"
        "    bindings = document.get('bindings', [])\n"
        "    if operation not in OPERATIONS or entrypoint != OPERATIONS[operation]:\n"
        "        raise SystemExit(1)\n"
        "    if document.get('operation') != operation or not bindings:\n"
        "        raise SystemExit(1)\n"
        "    if any(item.get('operation') != operation for item in bindings):\n"
        "        raise SystemExit(1)\n"
        "    print(json.dumps({\n"
